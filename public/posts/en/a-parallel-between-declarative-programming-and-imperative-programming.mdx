---
title: A Parallel Between Declarative Programming and Imperative Programming
...
As R. Kowalski stated in Communications of the ACM, Volume 22, Issue 7, July 1979, <High>an algorithm consists of logic and control flow. In declarative programming, the language tends to abstract the control flow, leaving the programmer to focus on developing the logic.</High> Formally, we can define:
...
In the article [_Practical Advantages of Declarative Programming_](https://www.cse.chalmers.se/~oloft/Papers/wm96.pdf), _Lloyd, J.W._ explains that <High>declarative programming can be understood as "**Strong**" and "**Weak**".</High> That is, <High color="blue">in the "Strong" sense, the programmer only provides the logic of what is needed.</High> <High color="red">In the "Weak" sense, the programmer needs to provide the logic and extend the control flow to compute the expected result.</High>
