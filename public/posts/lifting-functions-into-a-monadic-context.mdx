...
"The `>>` operator sequences two actions while discarding any resulting value of the first action⁶". For example, here the `>>` operator ignores the result of the first action (Just 5), and returns the result of the second action (Just "Second Action").
