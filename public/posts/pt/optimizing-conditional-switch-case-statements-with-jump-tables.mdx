...
Suponha que, **hipoteticamente**, que exista um `switch` com $15$ `cases` e ele é executado **dentro de um loop**. Na [notação assintótica](https://xlinux.nist.gov/dads/HTML/bigOnotation.html) do $Big\mathcal{O}$, temos um *loop* como sendo $\mathcal{O}(n)$ e $\mathcal{O}(15)$ `cases` que nos dá $\mathcal{O}(n*15)$ **no pior caso**; normalmente descartaríamos o 15 pois é um termo constante, mas para esse exemplo vamos considerar.
...
Mas e se pudêssemos tornar isso $\mathcal{O}(n*1)$ não seria melhor ? Imagine que esse loop fosse até $200$, então teríamos $\mathcal{O}(200*15)$ o que nos daria complexidade de $\mathcal{O}(3000)$ operações **no pior caso**, ou seja, onde **todos os 15 cases são condicionados**.

Agora, se conseguíssemos otimizar o código de tal forma que a quantidade de `cases` seja reduzida para $1$, a complexidade se torna $\mathcal{O}(n * 1)$, o que é efetivamente $\mathcal{O}(n)$ ? Pois bem, é aí que entra a jump table.
...
Ocorre que, <High color="red">o desvio condicional torna-se um desafio para manter o pipeline cheio, uma que vez que, quando uma instrução de branch é encontrada, a CPU não sabe antecipadamente qual caminho seguir</High>. Dependendo da condição associada ao branch, a CPU deve decidir se deve continuar com as instruções subsequentes no fluxo atual ou se deve pular para um novo fluxo de instruções. Atualmente existem algoritmos de **Branch Predictor** para pré-buscar essas instruções e colocar antecipadamente no pipeline, mas ainda assim não é obtido a melhor eficiência. E isso piora quando você tem branchs aninhadas, pois as branchs internas sequer são conhecidas para que se possa pré-buscar as instruções.
...
Por exemplo, a condicional no código abaixo, geralmente é mais lenta do que simplesmente `value = new_val` — não porque o código é maior — mas porque a variável poderia estar no _cache_ e o desvio condicional pode gerar um _branch misprediction_ ou _cache miss_.
...
<Info>
Um Array é um conjunto de itens acessíveis por números inteiros, o índice.
</Info>

<Info>
Um Dictionary (Dicionário) é um tipo de estrutura dados abstrata que armazena itens ou valores. Um valor é acessado por uma chave associada. As operações básicas são new, insert, find e delete.
</Info>

Os dicionários utilizam funções hash, que em casos de colisão de hash vem a ter uma complexidade $\mathcal{O}(n)$. Você encontrará exemplos na internet de jump tables com *dicionaries*, mas usualmente é implementada por compiladores por meio de arrays.

## Condições para uso de Jump Table (gcc)

- **Intervalo Contíguo de Casos**: se os valores dos <High>casos são contíguos ou próximos</High>, é mais eficiente usar uma jump table.

- **Número de Casos**: se o número de casos é suficiente para justificar a tabela, normalmente <High>5 ou mais casos contíguos</High>.

- **Case labels**: os rótulos dos casos devem ser constantes — não variáveis ou strings — <High>(1..n, a..z, Enum)</High>.
...
Adicionalmente, no exemplo seguinte temos uma branch table com `goto`. Mas como você pode notar, temos alguns operadores diferentes; a expressão toda declara um array de ponteiros para **labels**; as labels `Case1`, `Case2` e `Case3` são endereços de pontos específicos no código (**labels**) e o operador `&&` é uma extensão do GCC que permite **obter o endereço de uma label**.
