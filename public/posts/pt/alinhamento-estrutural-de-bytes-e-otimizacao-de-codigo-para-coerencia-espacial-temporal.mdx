---
title: Alinhamento Estrutural de Bytes e Otimização de Código para Coerência Espacial-Temporal
description: Como uma struct com menos bytes (mesmo que desalinhada) pode ser menos eficiente que uma struct com mais bytes (alinhada) ?
slug: alinhamento-estrutural-de-bytes-e-otimizacao-de-codigo-para-coerencia-espacial-temporal
image: 
alternativeText:
caption:
...
6. **Entender por que a Coerência espacial-temporal é importante para otimizar o desempenho do sistema computacional**.
...
Então teríamos $31 \ bytes$, quando alocada em uma memória fictícia que não existe alinhamento. Cada bloco representando $1 \ byte$, `operation` ocupando 1 bloco, `quantity` 2 blocos, `total` 8 blocos, `unitPrice` 4 blocos e `balance` 16 blocos. Isso se parece com a [Figura 1](#1).
...
$$
\begin{gathered}
Transaction_{t} = & \ 1 + 2 + 8 + 4 + 16 \\
Transaction_{p} = & \ 1 + 4 + 4 \\
Transaction = & \ 31 + 9 \\
Transaction = & \ 40 \text{ bytes}
\end{gathered}
$$
...
Então qual é a ideia aqui 🤔 ? <High color="red">A indicação é para que se **adicione pointer bytes por primeiro** na struct</High>.

## 3.5 Implicações

Você deve estar se perguntando: **"Por que uma struct com menos bytes (mesmo que desalinhada) possa ser menos eficiente que uma struct com mais bytes (alinhada)?"**. <High color="purple">Pois é, essa foi a pergunta que me fez escrever este post</High>.
