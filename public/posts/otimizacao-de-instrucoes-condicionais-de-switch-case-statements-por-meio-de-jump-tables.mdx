...
Ocorre que, <TextHighlight color="red">o desvio condicional torna-se um desafio para manter o pipeline cheio, uma que vez que, quando uma instrução de branch é encontrada, a CPU não sabe antecipadamente qual caminho seguir</TextHighlight>. Dependendo da condição associada ao branch, a CPU deve decidir se deve continuar com as instruções subsequentes no fluxo atual ou se deve pular para um novo fluxo de instruções. Atualmente existem algoritmos de **Branch Predictor** para pré-buscar essas instruções e colocar antecipadamente no pipeline, mas ainda assim não é obtido a melhor eficiência. E isso piora quando você tem branchs aninhadas, pois as branchs internas sequer são conhecidas para que se possa pré-buscar as instruções.
...
O Intervalo Contíguo de Casos: se os valores dos <TextHighlight>casos são contíguos ou próximos</TextHighlight>, é mais eficiente usar uma jump table.

O Número de Casos: se o número de casos é suficiente para justificar a tabela, normalmente <TextHighlight>5 ou mais casos contíguos</TextHighlight>.

Case labels: os rótulos dos casos devem ser constantes — não variáveis ou strings — <TextHighlight>(1..n, a..z, Enum)</TextHighlight>.
