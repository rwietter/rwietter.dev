...
Para este post, estarei utilizando uma arquitetura x86-64; como linguagem — Golang — para ilustrar os exemplos. Na seção **"Regras de alinhamento"** estão dispostos os tipos da linguagem e seus respectivos tamanhos e as regras de alinhamento do compilador.

### Diretivas de Alinhamento
...
#### Diretivas de Alinhamento Específicos
...
Fechando esse hiato de conhecimento, agora finalmente podemos ir para a parte prática e ver como isso se aplica ao nosso código. Na `Transaction` abaixo, defini alguns campos diversos com tipos e seus respectivos tamanhos.

Se você nunca viu sobre isso, deve estar bem confuso ainda — e com razão. Relaxa, eu já estive nesse lugar. Vamos ver um exemplo prático, até o final do post você vai entender tudo isso.
...
Nossa "memória mágica" pareceria algo assim. Cada bloco representando 1 byte, `operation` ocupando 1 bloco, `quantity` 2 blocos, `total` 8 blocos, `unitPrice` 4 blocos e `balance` 16 blocos.

![100%](/assets/magic-memory.png)

Em teoria, a `struct` `Transaction` deveria ter $31 \ bytes$, mas se verificarmos o tamanho da `struct` no Go, veremos que ela tem $40 \ bytes$. O que está acontecendo aqui? O que está errado? Bem, ocorre que, nossa `struct` não está alinhada, então o compilador adicionou lacunas extras (`padding`) para garantir que os campos estejam alinhados.
...
func main() {
	...
	for i := 0; i < t.NumField(); i++ {
		field := t.Field(i)
		fmt.Printf("Field: %s, Type: %s, Size: %d bytes\n", field.Name, field.Type, field.Type.Size())
	}
}
```
...
Como resultado de alinhamento, o go adicionou essas lacunas entre os campos e arrendondou o tamanho da `struct` para 40 bytes. Nossa memória agora parece algo assim:

![100%](/assets/memory-alignment-40-bytes.png)
...
5. **`Balance` (complex128):**  Como precisa de alinhamento de 8 bytes e o offset após `UnitPrice` já é múltiplo de 8, ele se encaixa perfeitamente sem a necessidade de padding adicional.
